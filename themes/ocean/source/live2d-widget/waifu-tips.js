...
function loadWidget(config) {
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			else if (now > 7 && now <= 11) text = "上午好！开始学习了嘛？注意不要久坐，多起来走动走动哦！";
			else if (now > 11 && now <= 13) text = ["中午了，工作了一个上午，现在是午餐时间！出门记得戴口罩哇！","太阳晒晒，出门记得带帽子或者太阳伞哦","饭后很容易犯困呢，但吃饱饭不要马上睡觉哦。"];
			else if (now > 13 && now <= 17) text = ["下午睡一觉精力更充沛喔~","记得多喝水嘿！"];
		...
		if (useCDN) {
			...
			fetch(`${apiPath}rand_textures/?id=${modelId}-${modelTexturesId}`)
				...
				.then(result => {
					...
					else loadModel(modelId, result.textures.id, "小魔仙全身变");
				});
		}
	}
